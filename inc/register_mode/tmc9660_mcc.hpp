 ...
 *
 * Provides raw ADC readings of on-chip temperature sensor and supply (V<sub>SA</sub>) voltage monitor.
 *
 * Register Map:
 * Bits    | Name      | Access | Description
 * --------|-----------|--------|-------------
 ...
 *
 * General configuration setup of the current ADCs, allowing control over various calibration and measurement settings.
 *
 * Register Map:
 * Bits    | Name            | Access | Description
 * --------|-----------------|--------|-------------
 ...
 *
 * Represents the status register of the TMC9660 driver, providing detailed information about the state of various ADC measurements and their completion status.
 *
 * Register Map:
 * Bits    | Name            | Access | Description
 * --------|-----------------|--------|-------------
 ...
 * Bits    | Name            | Access | Description
 * --------|-----------------|--------|-------------
 * 1:0     | SV_MODE         | RW     | Space Vector PWM mode (0: off, 1: 3rd harmonic injection, etc.)
 * 2       | POLARITY        | RW     | Invert PWM output polarity (1 = inverted)
 * 3       | CENTER_ALIGNED  | RW     | Center-aligned PWM if set, edge-aligned if clear
 * 7:4     | PWM_FREQ_DIV    | RW     | PWM frequency divider (sets base PWM frequency)
 *
...
struct ABN_CPR {
    ...
    uint32_t calculateInverseCPR() const {
        auto cpr = bits.CPR;
        return cpr
            ? static_cast<uint32_t>((uint64_t{1} << 32) / cpr)
            : 0u;
    }
};
 ...
 * Bits    | Name                    | Access | Description
 * --------|-------------------------|--------|-------------
 * 31:16   | PHI_E_EXTRAPOLATED      | R      | Extrapolated electrical angle hall_phi_e_extrapolated
 * 15:0    | PHI_E                   | R      | Electrical angle hall_phi_e
 *
 ...
 * Bits    | Name                        | Access | Description
 * --------|-----------------------------|--------|-------------
 * 7:0     | SELECTION                   | RW     | Source of rotor position for velocity measurement
 * 8       | METER_SYNC_PULSE            | RW     | Synchronization pulse for velocity meter
 * 10:9    | METER_TYPE                  | RW     | Velocity meter type selection
 * 14:12   | MOVING_AVRG_FILTER_SAMPLES  | RW     | Moving average filter samples
 *
...
/**
 * @brief Velocity Meter Minimal Deviation and Counter Limit Register (MCC_V_MIN_POS_DEV_TIME_COUNTER_LIMIT)
 ...
 * 15:0    | TIME_COUNTER_LIMIT  | RW     | Counter limit for velocity minimum deviation functionality
 *
 ...
 * Bits    | Name          | Access | Description
 * --------|---------------|--------|-------------
 * 26:0    | RAMPER_V1     | RW     | Velocity threshold for switching acceleration/deceleration values
 *
 ...
 * Bits    | Name          | Access | Description
 * --------|---------------|--------|-------------
 * 26:0    | RAMPER_V2     | RW     | Second velocity threshold for switching acceleration/deceleration values
 *
 ...
 * Bits    | Name                    | Access | Description
 * --------|-------------------------|--------|-------------
 * 0       | STOP_L_ENABLE           | RW     | Enables automatic motor stop during active left reference switch input
 * 1       | STOP_R_ENABLE           | RW     | Enables automatic motor stop during active right reference switch input
 * 2       | STOP_H_ENABLE           | RW     | Enables automatic motor stop during active home reference switch input
 * 3       | STOP_L_POL              | RW     | Defines active polarity of the left reference switch input
 * 4       | STOP_R_POL              | RW     | Defines active polarity of the right reference switch input
 * 5       | STOP_H_POL              | RW     | Defines active polarity of the home reference switch input
 * 6       | SWAP_LR                 | RW     | Swaps left and right reference switch inputs internally
 * 7       | LATCH_L_ACTIVE          | RW     | Activates position latching to RAMPER_X_ACTUAL_LATCH when left reference switch is activated
 * 8       | LATCH_L_INACTIVE        | RW     | Activates position latching to RAMPER_X_ACTUAL_LATCH when left reference switch is deactivated
 * 9       | LATCH_R_ACTIVE          | RW     | Activates position latching to RAMPER_X_ACTUAL_LATCH when right reference switch is activated
 * 10      | LATCH_R_INACTIVE        | RW     | Activates position latching to RAMPER_X_ACTUAL_LATCH when right reference switch is deactivated
 * 11      | LATCH_H_ACTIVE          | RW     | Activates position latching to RAMPER_X_ACTUAL_LATCH when home reference switch is activated
 * 12      | LATCH_H_INACTIVE        | RW     | Activates position latching to RAMPER_X_ACTUAL_LATCH when home reference switch is deactivated
 * 14      | SG_STOP_ENABLE          | RW     | Enables stop conditions like SW_HARD_STOP, STOP_ON_POS_DEVIATION, STOP_ON_VEL_DEVIATION
 * 15      | SOFTSTOP_ENABLE         | RW     | Enables soft stop mode using deceleration ramp settings
 * 16      | SW_HARD_STOP            | RW     | Enables a hard stop during ramp mode in case any activated reference switch has been triggered
 * 17      | STOP_ON_POS_DEVIATION   | RW     | Enables a hard stop during ramp mode if position tracking error emerges
 * 18      | STOP_ON_VEL_DEVIATION   | RW     | Enables a hard stop during ramp mode if velocity tracking error emerges
 * 19      | VELOCITY_OVERWRITE      | RW     | If enabled, velocity from overwrite input (PID_VELOCITY_TARGET) is written to ramp
 *
 ...
 * Bits    | Name              | Access | Description
 * --------|-------------------|--------|-------------
 * 15:0    | RAMPER_PHI_E      | R      | PHI_E calculated from RAMPER_X_ACTUAL × N_POLE_PAIRS + Offset
 *
 ...
 * Bits    | Name                      | Access | Description
 * --------|---------------------------|--------|-------------
 * 31:0    | POSITION_ACTUAL_LATCH     | R      | Actual feedback position latch at stop switch event
 *
 ...
 * Bits    | Name                | Access | Description
 * --------|---------------------|--------|-------------
 * 0       | KEEP_POS_TARGET     | RW     | Do not overwrite position target on position actual write
 * 2       | CURRENT_NORM_P      | RW     | Normalization of P Factor of Current Control
 * 3       | CURRENT_NORM_I      | RW     | Normalization of I Factor of Current Control
 * 5:4     | VELOCITY_NORM_P     | RW     | Normalization of P Factor of Velocity Control
 * 7:6     | VELOCITY_NORM_I     | RW     | Normalization of I Factor of Velocity Control
 * 9:8     | POSITION_NORM_P     | RW     | Normalization of P Factor of Position Control
 * 11:10   | POSITION_NORM_I     | RW     | Normalization of I Factor of Position Control
 * 15:12   | VEL_SCALE           | RW     | Output right shift factor of the velocity controller
 * 22:16   | POS_SMPL            | RW     | Downsampling factor for Position controller
 * 30:24   | VEL_SMPL            | RW     | Downsampling factor for Velocity controller
 *
 ...
 * Bits    | Name                      | Access | Description
 * --------|---------------------------|--------|-------------
 * 0       | PID_X_TARGET_LIMIT        | R      | PI position controller target value limiter active
 * 1       | PID_X_OUTPUT_LIMIT        | R      | PI position controller output limiter active
 * 2       | PID_V_TARGET_LIMIT        | R      | PI velocity controller target value limiter active
 * 3       | PID_V_OUTPUT_LIMIT        | R      | PI velocity controller output limiter active
 * 4       | PID_ID_TARGET_LIMIT       | R      | PI flux controller target value limiter active
 * 5       | PID_ID_OUTPUT_LIMIT       | R      | PI flux controller output limiter active
 * 6       | PID_IQ_TARGET_LIMIT       | R      | PI torque controller target value limiter active
 * 7       | PID_IQ_OUTPUT_LIMIT       | R      | PI torque controller output limiter active
 * 8       | IPARK_VOLTLIM_LIMIT_U     | R      | Ud or Uq limited by PID_UQ_UD_LIMITS or internal max value
 * 9       | PWM_SWITCH_LIMIT_ACTIVE   | R      | PWM switch limit active
 * 10      | HALL_ERROR                | R      | Hall vector error (000 or 111)
 * 11      | POSITION_TRACKING_ERROR   | R      | PI position controller error exceeds MAX_POS_DEVIATION
 * 12      | VELOCITY_TRACKING_ERROR   | R      | PI velocity controller error exceeds MAX_VEL_DEVIATION
 * 13      | PID_FW_OUTPUT_LIMIT       | R      | Field weakening PI controller output limit active
 * 16      | SHORT                     | R      | HS_FAULT or LS_FAULT triggered
 * 20      | REF_SW_L                  | R      | Left reference switch active
 * 21      | REF_SW_R                  | R      | Right reference switch active
 * 22      | REF_SW_H                  | R      | Home reference switch active
 * 23      | POSITION_REACHED          | R      | Ramper position reached
 * 26      | ADC_I_CLIPPED             | R      | ADC current measurement clipped
 * 28      | ENC_N                     | R      | Filtered encoder signal N active
 * 31      | ENI                       | R      | Change on DRV_ENABLE pin detected
 *
 ...
 * Bits    | Name                      | Access | Description
 * --------|---------------------------|--------|-------------
 * 31      | VS_UVLO_PROT              | RW     | Disables the gate driver on VS undervoltage event
 * 29      | VDRV_UVLO_PROT            | RW     | Disables the gate driver on VDRV undervoltage event
 * 27      | HS_VGS_ON_SHORT_PROT_Y2   | RW     | Disables the gate driver if a gate short event occurs while the gate is turned on (Y2 phase)
 * 26      | HS_VGS_ON_SHORT_PROT_W    | RW     | Disables the gate driver if a gate short event occurs while the gate is turned on (W phase)
 * 25      | HS_VGS_ON_SHORT_PROT_V    | RW     | Disables the gate driver if a gate short event occurs while the gate is turned on (V phase)
 * 24      | HS_VGS_ON_SHORT_PROT_U    | RW     | Disables the gate driver if a gate short event occurs while the gate is turned on (U phase)
 * 23      | HS_VGS_OFF_SHORT_PROT_Y2  | RW     | Disables the gate driver if a gate short event occurs while the gate is turned off (Y2 phase)
 * 22      | HS_VGS_OFF_SHORT_PROT_W   | RW     | Disables the gate driver if a gate short event occurs while the gate is turned off (W phase)
 * 21      | HS_VGS_OFF_SHORT_PROT_V   | RW     | Disables the gate driver if a gate short event occurs while the gate is turned off (V phase)
 * 20      | HS_VGS_OFF_SHORT_PROT_U   | RW     | Disables the gate driver if a gate short event occurs while the gate is turned off (U phase)
 * 19      | HS_SHORT_PROT_Y2          | RW     | Disables the channel if an overcurrent event occurs (Y2 phase)
 * 18      | HS_SHORT_PROT_W           | RW     | Disables the channel if an overcurrent event occurs (W phase)
 * 17      | HS_SHORT_PROT_V           | RW     | Disables the channel if an overcurrent event occurs (V phase)
 * 16      | HS_SHORT_PROT_U           | RW     | Disables the channel if an overcurrent event occurs (U phase)
 * 15      | BST_UVLO_PROT_Y2          | RW     | Disables the affected phase if a bootstrap capacitor undervoltage event occurs (Y2 phase)
 * 14      | BST_UVLO_PROT_W           | RW     | Disables the affected phase if a bootstrap capacitor undervoltage event occurs (W phase)
 * 13      | BST_UVLO_PROT_V           | RW     | Disables the affected phase if a bootstrap capacitor undervoltage event occurs (V phase)
 * 12      | BST_UVLO_PROT_U           | RW     | Disables the affected phase if a bootstrap capacitor undervoltage event occurs (U phase)
 * 11      | LS_VGS_ON_SHORT_PROT_Y2   | RW     | Disables the gate driver if a gate short event occurs while the gate is turned on (Y2 phase)
 * 10      | LS_VGS_ON_SHORT_PROT_W    | RW     | Disables the gate driver if a gate short event occurs while the gate is turned on (W phase)
 * 9       | LS_VGS_ON_SHORT_PROT_V    | RW     | Disables the gate driver if a gate short event occurs while the gate is turned on (V phase)
 * 8       | LS_VGS_ON_SHORT_PROT_U    | RW     | Disables the gate driver if a gate short event occurs while the gate is turned on (U phase)
 * 7       | LS_VGS_OFF_SHORT_PROT_Y2  | RW     | Disables the gate driver if a gate short event occurs while the gate is turned off (Y2 phase)
 * 6       | LS_VGS_OFF_SHORT_PROT_W   | RW     | Disables the gate driver if a gate short event occurs while the gate is turned off (W phase)
 * 5       | LS_VGS_OFF_SHORT_PROT_V   | RW     | Disables the gate driver if a gate short event occurs while the gate is turned off (V phase)
 * 4       | LS_VGS_OFF_SHORT_PROT_U   | RW     | Disables the gate driver if a gate short event occurs while the gate is turned off (U phase)
 * 3       | LS_SHORT_PROT_Y2          | RW     | Disables the channel if an overcurrent event occurs (Y2 phase)
 * 2       | LS_SHORT_PROT_W           | RW     | Disables the channel if an overcurrent event occurs (W phase)
 * 1       | LS_SHORT_PROT_V           | RW     | Disables the channel if an overcurrent event occurs (V phase)
 * 0       | LS_SHORT_PROT_U           | RW     | Disables the channel if an overcurrent event occurs (U phase)
 *
 ...
 *
 * Represents the STATUS_INT_ENABLE register configuration for enabling reporting or actions for various fault conditions in a motor driver.
 *
 * Register Map:
 * Bits    | Name                      | Access | Description
 * --------|---------------------------|--------|-------------
 * 0       | LS_SHORT_EN_U             | RW     | Enables reporting/action for overcurrent event (U phase)
 * 1       | LS_SHORT_EN_V             | RW     | Enables reporting/action for overcurrent event (V phase)
 * 2       | LS_SHORT_EN_W             | RW     | Enables reporting/action for overcurrent event (W phase)
 * 3       | LS_SHORT_EN_Y2            | RW     | Enables reporting/action for overcurrent event (Y2 phase)
 * 4       | LS_VGS_OFF_SHORT_EN_U     | RW     | Enables reporting/action for gate short event while off (U phase)
 * 5       | LS_VGS_OFF_SHORT_EN_V     | RW     | Enables reporting/action for gate short event while off (V phase)
 * 6       | LS_VGS_OFF_SHORT_EN_W     | RW     | Enables reporting/action for gate short event while off (W phase)
 * 7       | LS_VGS_OFF_SHORT_EN_Y2    | RW     | Enables reporting/action for gate short event while off (Y2 phase)
 * 8       | LS_VGS_ON_SHORT_EN_U      | RW     | Enables reporting/action for gate short event while on (U phase)
 * 9       | LS_VGS_ON_SHORT_EN_V      | RW     | Enables reporting/action for gate short event while on (V phase)
 * 10      | LS_VGS_ON_SHORT_EN_W      | RW     | Enables reporting/action for gate short event while on (W phase)
 * 11      | LS_VGS_ON_SHORT_EN_Y2     | RW     | Enables reporting/action for gate short event while on (Y2 phase)
 * 12      | BST_UVLO_EN_U             | RW     | Enables reporting/action for bootstrap undervoltage (U phase)
 * 13      | BST_UVLO_EN_V             | RW     | Enables reporting/action for bootstrap undervoltage (V phase)
 * 14      | BST_UVLO_EN_W             | RW     | Enables reporting/action for bootstrap undervoltage (W phase)
 * 15      | BST_UVLO_EN_Y2            | RW     | Enables reporting/action for bootstrap undervoltage (Y2 phase)
 * 16      | HS_SHORT_EN_U             | RW     | Enables reporting/action for overcurrent event (U phase)
 * 17      | HS_SHORT_EN_V             | RW     | Enables reporting/action for overcurrent event (V phase)
 * 18      | HS_SHORT_EN_W             | RW     | Enables reporting/action for overcurrent event (W phase)
 * 19      | HS_SHORT_EN_Y2            | RW     | Enables reporting/action for overcurrent event (Y2 phase)
 * 20      | HS_VGS_OFF_SHORT_EN_U     | RW     | Enables reporting/action for gate short event while off (U phase)
 * 21      | HS_VGS_OFF_SHORT_EN_V     | RW     | Enables reporting/action for gate short event while off (V phase)
 * 22      | HS_VGS_OFF_SHORT_EN_W     | RW     | Enables reporting/action for gate short event while off (W phase)
 * 23      | HS_VGS_OFF_SHORT_EN_Y2    | RW     | Enables reporting/action for gate short event while off (Y2 phase)
 * 24      | HS_VGS_ON_SHORT_EN_U      | RW     | Enables reporting/action for gate short event while on (U phase)
 * 25      | HS_VGS_ON_SHORT_EN_V      | RW     | Enables reporting/action for gate short event while on (V phase)
 * 26      | HS_VGS_ON_SHORT_EN_W      | RW     | Enables reporting/action for gate short event while on (W phase)
 * 27      | HS_VGS_ON_SHORT_EN_Y2     | RW     | Enables reporting/action for gate short event while on (Y2 phase)
 * 28      | VDRV_UVLO_EN              | RW     | Enables reporting/action for VDRV undervoltage event
 * 29      | VDRV_UVLWRN_EN            | RW     | Enables reporting/action for VDRV undervoltage warning
 * 30      | VS_UVLO_EN                | RW     | Enables reporting/action for VS undervoltage event
 *
 ...
 * Bits    | Name                      | Access | Description
 * --------|---------------------------|--------|-------------
 * 0       | LS_SHORT_U                | R      | Status of the overcurrent protection (low-side, U phase)
 * 1       | LS_SHORT_V                | R      | Status of the overcurrent protection (low-side, V phase)
 * 2       | LS_SHORT_W                | R      | Status of the overcurrent protection (low-side, W phase)
 * 3       | LS_SHORT_Y2               | R      | Status of the overcurrent protection (low-side, Y2 phase)
 * 4       | LS_VGS_OFF_SHORT_U        | R      | Gate short detection while off (low-side, U phase)
 * 5       | LS_VGS_OFF_SHORT_V        | R      | Gate short detection while off (low-side, V phase)
 * 6       | LS_VGS_OFF_SHORT_W        | R      | Gate short detection while off (low-side, W phase)
 * 7       | LS_VGS_OFF_SHORT_Y2       | R      | Gate short detection while off (low-side, Y2 phase)
 * 8       | LS_VGS_ON_SHORT_U         | R      | Gate short detection while on (low-side, U phase)
 * 9       | LS_VGS_ON_SHORT_V         | R      | Gate short detection while on (low-side, V phase)
 * 10      | LS_VGS_ON_SHORT_W         | R      | Gate short detection while on (low-side, W phase)
 * 11      | LS_VGS_ON_SHORT_Y2        | R      | Gate short detection while on (low-side, Y2 phase)
 * 12      | BST_UVLO_U                | R      | Undervoltage condition of the bootstrap cap (U phase)
 * 13      | BST_UVLO_V                | R      | Undervoltage condition of the bootstrap cap (V phase)
 * 14      | BST_UVLO_W                | R      | Undervoltage condition of the bootstrap cap (W phase)
 * 15      | BST_UVLO_Y2               | R      | Undervoltage condition of the bootstrap cap (Y2 phase)
 * 16      | HS_SHORT_U                | R      | Status of the overcurrent protection (high-side, U phase)
 * 17      | HS_SHORT_V                | R      | Status of the overcurrent protection (high-side, V phase)
 * 18      | HS_SHORT_W                | R      | Status of the overcurrent protection (high-side, W phase)
 * 19      | HS_SHORT_Y2               | R      | Status of the overcurrent protection (high-side, Y2 phase)
 * 20      | HS_VGS_OFF_SHORT_U        | R      | Gate short detection while off (high-side, U phase)
 * 21      | HS_VGS_OFF_SHORT_V        | R      | Gate short detection while off (high-side, V phase)
 * 22      | HS_VGS_OFF_SHORT_W        | R      | Gate short detection while off (high-side, W phase)
 * 23      | HS_VGS_OFF_SHORT_Y2       | R      | Gate short detection while off (high-side, Y2 phase)
 * 24      | HS_VGS_ON_SHORT_U         | R      | Gate short detection while on (high-side, U phase)
 * 25      | HS_VGS_ON_SHORT_V         | R      | Gate short detection while on (high-side, V phase)
 * 26      | HS_VGS_ON_SHORT_W         | R      | Gate short detection while on (high-side, W phase)
 * 27      | HS_VGS_ON_SHORT_Y2        | R      | Gate short detection while on (high-side, Y2 phase)
 * 28      | VDRV_UVLO                 | R      | Undervoltage condition of VDRV (gate drive voltage)
 * 29      | VDRV_UVLWRN               | R      | Low voltage warning condition of VDRV (gate drive voltage)
 * 30      | VS_UVLO                   | R      | Undervoltage condition of VS (supply voltage)
 *
 ...
 * 30      | VDRV_UVLWRN_STS           | R      | Low voltage warning condition of VDRV (gate drive voltage)
 * 29      | VDRV_UVLO_STS             | R      | Undervoltage condition of VDRV (gate drive voltage)
 * 19      | HS_FAULT_ACTIVE_Y2        | RW     | Set if any fault occurred on phase Y2 that triggered hardware protection
 * 18      | HS_FAULT_ACTIVE_W         | RW     | Set if any fault occurred on phase W that triggered hardware protection
 * 17      | HS_FAULT_ACTIVE_V         | RW     | Set if any fault occurred on phase V that triggered hardware protection
 * 16      | HS_FAULT_ACTIVE_U         | RW     | Set if any fault occurred on phase U that triggered hardware protection
 * 15      | BST_UVLO_STS_Y2           | R      | Undervoltage condition of the bootstrap capacitor on phase Y2
 * 14      | BST_UVLO_STS_W            | R      | Undervoltage condition of the bootstrap capacitor on phase W
 * 13      | BST_UVLO_STS_V            | R      | Undervoltage condition of the bootstrap capacitor on phase V
 * 12      | BST_UVLO_STS_U            | R      | Undervoltage condition of the bootstrap capacitor on phase U
 * 3       | LS_FAULT_ACTIVE_Y2        | RW     | Set if any fault occurred on phase Y2 that triggered hardware protection
 * 2       | LS_FAULT_ACTIVE_W         | RW     | Set if any fault occurred on phase W that triggered hardware protection
 * 1       | LS_FAULT_ACTIVE_V         | RW     | Set if any fault occurred on phase V that triggered hardware protection
 * 0       | LS_FAULT_ACTIVE_U         | RW     | Set if any fault occurred on phase U that triggered hardware protection
 *
 * @note This register provides the status of various fault events in the system
 */
struct FAULT {
 ...
 *
 * @note This register provides the external writable parameters for open-loop voltage control mode
 */
struct VOLTAGE_EXT {
