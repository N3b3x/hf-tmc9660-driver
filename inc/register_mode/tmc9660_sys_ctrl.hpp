 ...
 * Each bit indicates a specific fault or status condition. Bits are set and cleared automatically
 * by hardware.
 *
 * Register Map:
 ...
 * LDOEXT1_SHORT    | R      | 0x0   | LDO1 (V_EXT1) is shorted 2       | LDOEXT_TSD       | R | 0x0
 * | LDO thermal shutdown status 1       | BCK_SHORT        | R      | 0x0   | VBUCK shorted 0 |
 * BCK_UVLO         | R      | 0x0   | VBUCK undervoltage lockout detected
 *
 ...
 *
 * Read/Write register of latched fault flags. Bits are set automatically if the corresponding bit
 * in FAULT_STATUS is set. Each bit must be cleared manually by writing 1 (W1C: Write 1 to Clear).
 * Bits remain set until explicitly cleared by software.
 *
 * Register Map:
 * Bits    | Name               | Access   | Reset | Description
 * --------|--------------------|----------|-------|----------------------------------------------------------
 ...
 *
 * @note All bits are set by hardware and must be cleared by writing 1. UC_FAULT can be set/cleared
 * by software.
 */
 ...
 *
 * Mask register for FAULT_STATUS_LATCHED (FAULT_R_INT). If a bit in this register is set and the
 * corresponding latched fault flag is set, the FAULTN pin will be asserted. Each bit enables or
 * disables the fault pin response for the corresponding latched fault. Default reset values are
 * shown below.
 *
 * Register Map:
 * Bits    | Name                   | Access | Reset | Description
