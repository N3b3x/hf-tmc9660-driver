...
TMC9660::bootloaderInit(const tmc9660::BootloaderConfig *cfg, bool performReset, 
  ...
  // ======================================================================
  if (performReset) {
    TMC9660_LOG_INFO("TMC9660", "Performing hardware reset sequence...");
    
    // 1.1: Assert RST (active high) to enter reset state
    if (!comm_.gpioSetActive(TMC9660CtrlPin::RST)) {
      TMC9660_LOG_ERROR("TMC9660", "Failed to assert RST pin");
      return BootloaderInitResult::Failure;
    }
    ...
    if (!comm_.gpioSetInactive(TMC9660CtrlPin::RST)) {
      TMC9660_LOG_ERROR("TMC9660", "Failed to release RST pin");
      return BootloaderInitResult::Failure;
    }
    
    // 1.3: Wait for FAULTN to go inactive (chip ready)
    TMC9660_LOG_DEBUG("TMC9660", "Waiting for FAULTN to go inactive (chip ready)...");
    ...
    for (int i = 0; i < 100; ++i) {  // Timeout after 1 second
      ...
      if (!comm_.gpioRead(TMC9660CtrlPin::FAULTN, faultnSignal)) {
        TMC9660_LOG_WARNING("TMC9660", "⚠️  Failed to read FAULTN pin - continuing anyway");
      ...
      if (faultnSignal == GpioSignal::INACTIVE) {
        chipReady = true;
        TMC9660_LOG_INFO("TMC9660", "✅ FAULTN inactive - chip ready after %d ms", i * 10);
        break;
      }
      comm_.delayMs(10);
    }
    ...
    if (!chipReady) {
      TMC9660_LOG_WARNING("TMC9660", "⚠️  FAULTN did not go inactive within timeout - continuing anyway");
      TMC9660_LOG_WARNING("TMC9660", "   (FAULTN may not be configured or chip may be in different state)");
    }
    
    // Small delay for chip to fully stabilize in bootloader
    comm_.delayMs(50);
  } else {
    TMC9660_LOG_INFO("TMC9660", "Skipping hardware reset (performReset=false)");
  }

  // ======================================================================
  // STEP 2: Detect Current Mode (Bootloader vs Parameter Mode)
  // ======================================================================
  TMC9660_LOG_INFO("TMC9660", "Detecting chip mode (bootloader vs parameter)...");
  ...
  if (comm_.mode() == CommMode::UART) {
    comm_.delayMs(100);
    TMC9660_LOG_DEBUG("TMC9660", "UART mode: Added 100ms stabilization delay");
  }
  ...
  TMC9660_LOG_DEBUG("TMC9660", "First reply raw status: 0x%02X (TMCL parse %s)",
  ...
  if (comm_.mode() == CommMode::UART && !tmclSuccess) {
    ...
    TMC9660_LOG_INFO("TMC9660", "Chip in BOOTLOADER mode (UART: no TMCL response)");
  }
  else if (rawStatus == 0x13) {
    inBootloaderMode = true;
    uint32_t version = firstReply.extractRawValue(1);
    TMC9660_LOG_INFO("TMC9660", "Chip in BOOTLOADER mode (SESSION_START), version: %d.%d",
    ...
    TMC9660_LOG_INFO("TMC9660", "Chip in PARAMETER mode (FIRST_CMD), version: %d.%d",
                  (version >> 16) & 0xFFFF, version & 0xFFFF);
  }
  else if (rawStatus == 0xFF && tmclSuccess && (firstReply.status == 100 || firstReply.status == 101)) {
    inParameterMode = true;
    TMC9660_LOG_INFO("TMC9660", "Chip in PARAMETER mode (initialized)");
  }
  else if (rawStatus == 0x00 && comm_.mode() == CommMode::SPI) {
    // For SPI: 0x00 can indicate bootloader OK status
    inBootloaderMode = true;
    TMC9660_LOG_INFO("TMC9660", "Chip in BOOTLOADER mode (SPI: OK status 0x00)");
  }
  else {
    TMC9660_LOG_ERROR("TMC9660", "Unable to detect chip mode (unknown status: 0x%02X, TMCL %s)",
                  rawStatus, tmclSuccess ? "succeeded" : "failed");
    return BootloaderInitResult::Failure;
  }
  ...
  if (inParameterMode && !inBootloaderMode) {
    TMC9660_LOG_INFO("TMC9660", "Chip in parameter mode, transitioning to bootloader...");
    
    // Send Boot command to enter bootloader
    if (!enterBootloaderMode()) {
      TMC9660_LOG_ERROR("TMC9660", "Failed to send Boot command");
      return BootloaderInitResult::Failure;
    }
    ...
    if (!bootloader_->noOp(&replyAfterBoot)) {
      TMC9660_LOG_ERROR("TMC9660", "Failed to verify bootloader mode after Boot command");
      return BootloaderInitResult::Failure;
    }
    
    uint8_t statusAfterBoot = static_cast<uint8_t>((replyAfterBoot >> 24) & 0xFF);
    if (statusAfterBoot != 0x15) {  // BOOTLOADER_RESUMED status
      TMC9660_LOG_WARNING("TMC9660", "⚠️  Expected BOOTLOADER_RESUMED (0x15), got 0x%02X", statusAfterBoot);
    } else {
      TMC9660_LOG_INFO("TMC9660", "✅ Successfully entered bootloader mode");
    }
  ...
  if (inBootloaderMode || !inParameterMode) {
    ...
    if (retrieveBootloaderInfo) {
      TMC9660_LOG_INFO("TMC9660", "Retrieving bootloader information...");
      if (!bootloader_->getAllBootloaderInfo()) {
        TMC9660_LOG_WARNING("TMC9660", "⚠️  Failed to retrieve some bootloader information");
      }
    }
    else {
      TMC9660_LOG_INFO("TMC9660", "Skipping bootloader information retrieval (retrieveBootloaderInfo=false)");
    }

    TMC9660_LOG_INFO("TMC9660", "Applying bootloader configuration...");
    if (!bootloader_->applyConfiguration(*useCfg, failOnVerifyError)) {
      TMC9660_LOG_ERROR("TMC9660", "Failed to apply bootloader configuration");
      return BootloaderInitResult::Failure;
    }
    TMC9660_LOG_INFO("TMC9660", "✅ Bootloader configuration applied successfully");
  }
  ...
  if (!useCfg->boot.start_motor_control) {
    TMC9660_LOG_INFO("TMC9660", "✅ Bootloader initialization complete (motor control NOT started)");
    TMC9660_LOG_INFO("TMC9660", "   cfg.boot.start_motor_control=false - chip remains in bootloader mode");
    TMC9660_LOG_INFO("TMC9660", "   Call bootloader_->startMotorControl() when ready");
  ...
  TMC9660_LOG_INFO("TMC9660", "✅ Bootloader initialization complete");
  TMC9660_LOG_INFO("TMC9660", "   Motor control started automatically via cfg.boot.start_motor_control=true");
  
  TMC9660_LOG_INFO("TMC9660", "⚠️  Waiting for initialization...");
  comm_.delayMs(100);
  ...
  if (comm_.mode() == CommMode::SPI) {
    TMC9660_LOG_INFO("TMC9660", "SPI mode: Waiting for SESSION_START and consuming status codes...");
    ...
    for (int attempt = 0; attempt < 50; ++attempt) {  // Timeout after 5 seconds (50 * 100ms)
      ...
      TMC9660_LOG_DEBUG("TMC9660", "Attempt %d/50: Current reply raw status: 0x%02X", attempt + 1, rawStatus2);
      
      if (rawStatus2 == 0x0C) {
        TMC9660_LOG_INFO("TMC9660", "✅ Parameter mode SESSION_START (0x0C) received after %d attempts", attempt + 1);
        sessionStartReceived = true;
        break;
      } else if (rawStatus2 == 0xFF) {
        TMC9660_LOG_INFO("TMC9660", "✅ SPI_STATUS_OK (0xFF) received after %d attempts - chip ready", attempt + 1);
        sessionStartReceived = true;
        break;
      } else if (rawStatus2 == 0xF0) {
        TMC9660_LOG_DEBUG("TMC9660", "SPI_STATUS_NOT_READY (0xF0) - waiting...");
        comm_.delayMs(100);
        continue;
      } else if (rawStatus2 == 0x13) {
        TMC9660_LOG_WARNING("TMC9660", "⚠️  Still in bootloader mode (0x13) - motor control not ready yet");
        comm_.delayMs(100);
        continue;
      } else if (rawStatus2 == 0x00) {
        TMC9660_LOG_WARNING("TMC9660", "⚠️  Bootloader OK (0x00) - motor control not ready yet");
        comm_.delayMs(100);
        continue;
      } else {
        TMC9660_LOG_WARNING("TMC9660", "⚠️  Unexpected status 0x%02X - waiting...", rawStatus2);
        comm_.delayMs(100);
        continue;
      }
    }
    ...
    if (!sessionStartReceived) {
      TMC9660_LOG_ERROR("TMC9660", "❌ Timeout waiting for SESSION_START or SPI_STATUS_OK");
      TMC9660_LOG_ERROR("TMC9660", "   Motor control may not have started properly");
  return BootloaderInitResult::Failure;
    }
  } else {
    // UART mode: No SESSION_START or status codes, just wait for motor control to initialize
    TMC9660_LOG_INFO("TMC9660", "UART mode: Waiting for motor control initialization...");
    comm_.delayMs(100);  // Give motor control time to start (adjust as needed)
    TMC9660_LOG_INFO("TMC9660", "✅ Motor control initialization delay completed");
  }
  
  // ======================================================================
  // STEP 8: Verify TMCL Communication with GetVersion
  // ======================================================================
  TMC9660_LOG_INFO("TMC9660", "Verifying TMCL communication with GetVersion...");
  
  // First, for UART mode, let's verify we're NOT still in bootloader mode
  if (comm_.mode() == CommMode::UART) {
    TMC9660_LOG_INFO("TMC9660", "UART mode: Testing if chip is still in bootloader mode...");
    tmc9660::BootloaderVersion bootloaderVer;
    if (bootloader_->getBootloaderVersion(&bootloaderVer)) {
      TMC9660_LOG_ERROR("TMC9660", "❌ ERROR: Chip is STILL in bootloader mode!");
      TMC9660_LOG_ERROR("TMC9660", "   Bootloader version: %d.%d", 
                     bootloaderVer.major, bootloaderVer.minor);
      TMC9660_LOG_ERROR("TMC9660", "   Motor control did NOT start properly");
      TMC9660_LOG_ERROR("TMC9660", "   Check boot configuration: cfg.boot.start_motor_control should be true");
      return BootloaderInitResult::Failure;
    } else {
      TMC9660_LOG_INFO("TMC9660", "✅ Bootloader commands fail (good - we're in parameter mode)");
    }
  }
  
  // Try a simple command first (MST - Motor Stop)
  TMC9660_LOG_INFO("TMC9660", "Testing TMCL communication with MST (Motor Stop)...");
  if (!sendCommand(tmc9660::tmcl::Op::MST, 0, 0, 0, nullptr)) {
    TMC9660_LOG_ERROR("TMC9660", "❌ TMCL communication failed (MST command failed)");
    TMC9660_LOG_ERROR("TMC9660", "   Motor control is not responding to TMCL commands");
    return BootloaderInitResult::Failure;
  }
  TMC9660_LOG_INFO("TMC9660", "✅ MST command successful - TMCL communication working!");
  
  // Now try GetVersion
  TMC9660_LOG_INFO("TMC9660", "Getting firmware version with GetVersion...");
  uint32_t version = 0;
  if (!sendCommand(tmc9660::tmcl::Op::GetVersion, 0, 0, 0, &version)) {
    TMC9660_LOG_ERROR("TMC9660", "❌ GetVersion failed (but MST worked)");
    TMC9660_LOG_ERROR("TMC9660", "   Basic TMCL works, but GetVersion has issues");
    return BootloaderInitResult::Failure;
  }
  
  // GetVersion success is already logged in sendCommand() with the version string
  TMC9660_LOG_INFO("TMC9660", "✅ TMCL communication verified - GetVersion successful");
  TMC9660_LOG_INFO("TMC9660", "✅ Chip fully initialized and ready for motor control commands");
...
bool TMC9660::sendCommand(tmc9660::tmcl::Op opcode, uint16_t type, uint8_t motor, uint32_t value,
  ...
  TMC9660_LOG_DEBUG("TMC9660", "[TMCL TX] Op=0x%02X, Type=0x%04X, Motor=0x%02X, Value=0x%08X",
                 tx.opcode, tx.type, tx.motor, tx.value);

  TMCLReply rep{};
  if (!comm_.transferTMCL(tx, rep, address_)) {
    TMC9660_LOG_WARNING("TMC9660", "[TMCL] Transfer failed");
    return false;
  }
  
  // Debug logging for TMCL reply
  TMC9660_LOG_DEBUG("TMC9660", "[TMCL RX] Status=0x%02X, Value=0x%08X", rep.status, rep.value);
  ...
  if (opcode == tmc9660::tmcl::Op::GetVersion && type == 0) {
    ...
    if (!versionString.empty()) {
      TMC9660_LOG_INFO("TMC9660", "✅ GetVersion successful - Firmware version: %s", versionString.c_str());
      if (reply) *reply = 0;  // No numeric value for string format
      return true;
    } else {
      TMC9660_LOG_WARNING("TMC9660", "⚠️  GetVersion string format not detected, trying standard parsing");
    }
  }
  
  if (!rep.isOK()) {
    TMC9660_LOG_INFO("TMC9660", "[TMCL] Reply not OK (status=0x%02X)", rep.status);
    return false;
  }
  if (reply)
    *reply = rep.value;
...
bool TMC9660::FOCControl::setCurrentLoopGains(uint16_t p, uint16_t i, bool separate, uint16_t fluxP,
...
bool TMC9660::FOCControl::setVelocitySensor(tmc9660::tmcl::VelocitySensorSelection sel) noexcept {
  return driver.writeParameter(tmc9660::tmcl::Parameters::VELOCITY_SENSOR_SELECTION,
                               static_cast<uint32_t>(sel));
}

bool TMC9660::FOCControl::getVelocitySensor(tmc9660::tmcl::VelocitySensorSelection &sel) noexcept {
  uint32_t v;
...
bool TMC9660::FOCControl::setStopOnVelocityDeviation(uint32_t maxError, bool softStop) noexcept {
  bool ok = true;
...
bool TMC9660::FOCControl::getStopOnVelocityDeviation(uint32_t &maxError, bool &softStop) noexcept {
  uint32_t v;
...
bool TMC9660::FOCControl::getVelocityMeterMode(tmc9660::tmcl::VelocityMeterMode &mode) noexcept {
  uint32_t v;
...
bool TMC9660::FOCControl::setPositionSensor(tmc9660::tmcl::PositionSensorSelection sel) noexcept {
  return driver.writeParameter(tmc9660::tmcl::Parameters::POSITION_SENSOR_SELECTION,
                               static_cast<uint32_t>(sel));
}

bool TMC9660::FOCControl::getPositionSensor(tmc9660::tmcl::PositionSensorSelection &sel) noexcept {
  uint32_t v;
...
bool TMC9660::FOCControl::setStopOnPositionDeviation(uint32_t maxError, bool softStop) noexcept {
  bool ok = true;
...
bool TMC9660::FOCControl::getStopOnPositionDeviation(uint32_t &maxError, bool &softStop) noexcept {
  uint32_t v;
...
bool TMC9660::FOCControl::setEventStopSettings(tmc9660::tmcl::EventStopSettings settings) noexcept {
